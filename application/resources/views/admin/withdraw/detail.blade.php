    ...
    <div class="row mb-none-30">
        ...
        <div class="col-lg-4 col-md-4 mb-30">
            <div class="card p-16 radius--base br--solid bg--white overflow-hidden">
                <h5 class="border-bottom pb-2">@lang('Withdraw Via') {{ __($withdrawal->method?->name ?? '') }}</h5>
                <ul class="list-group pt-4">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Date')
                        <span class="fw--500">{{ showDateTime($withdrawal->created_at) }}</span>
                    </li>

                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Trx Number')
                        <span class="fw--500">{{ $withdrawal->trx }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Username')
                        <span class="fw--500">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Method')
                        <span class="fw--500">{{ __($withdrawal->method->name) }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Amount')
                        <span class="fw--500">{{ showAmount($withdrawal->amount) }} {{ __($general->cur_text) }}</span>
                    </li>

                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Charge')
                        <span class="fw--500">{{ showAmount($withdrawal->charge) }} {{ __($general->cur_text) }}</span>
                    </li>

                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('After Charge')
                        <span class="fw--500">{{ showAmount($withdrawal->after_charge) }}
                            {{ __($general->cur_text) }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Rate')
                        <span class="fw--500">1 {{ __($general->cur_text) }}
                            = {{ showAmount($withdrawal->rate) }} {{ __($withdrawal->currency) }}</span>
                    </li>

                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Payable')
                        <span class="fw--500">{{ showAmount($withdrawal->final_amount) }}
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.withdraw.approve') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>@lang('Have you sent') <span class="fw--500 withdraw-amount text-success"></span>?</p>
